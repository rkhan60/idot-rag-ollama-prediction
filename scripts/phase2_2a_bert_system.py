...
class Phase22aBERTSystem:
    ...
    def build_rag_knowledge_base(self):
        """Phase 2.2a: Build advanced RAG knowledge base with BERT embeddings"""
        print("Building advanced RAG knowledge base with BERT embeddings...")
        
        # Initialize BERT model
        print("Loading BERT model...")
        self.bert_model = SentenceTransformer('all-MiniLM-L6-v2')  # Fast and effective model
        ...
        print(f"Generating BERT embeddings for {len(project_descriptions)} projects...")
        self.project_embeddings = self.bert_model.encode(project_descriptions, show_progress_bar=True)
        self.project_metadata = project_metadata
        
        print(f"Built advanced RAG knowledge base with BERT embeddings for {len(project_descriptions)} projects")
    
    def retrieve_similar_projects_enhanced(self, project_description, project_context, top_k=5):
        """Phase 2.2a: Advanced RAG with BERT embeddings and temporal weighting"""
        if self.project_embeddings is None or self.bert_model is None:
            return []
        
        # Generate query embedding
        query_embedding = self.bert_model.encode([project_description])
        
        # Calculate cosine similarities
        similarities = cosine_similarity(query_embedding, self.project_embeddings).flatten()
        ...
        for idx in top_indices:
            if similarities[idx] > 0.1:
                ...
                # Phase 2.2a: Enhanced similarity calculation with BERT
                ...
                similar_projects.append({
                    ...
                    'bert_similarity': base_similarity  # Track BERT-specific similarity
                })
    ...
    def calculate_phase2_score(self, firm_info, project_details, similar_projects):
        ...
        # Phase 2.2a: BERT similarity bonus
        bert_bonus = 0
        if similar_projects:
            avg_bert_similarity = np.mean([sp['bert_similarity'] for sp in similar_projects])
            if avg_bert_similarity > 0.3:  # Higher BERT similarity threshold
                bert_bonus = 5 + np.random.randint(0, 5)
        
        # Calculate final score
        final_score = (base_score - distance_penalty + capacity_bonus + 
                      historical_bonus + recent_bonus + similar_project_bonus + 
                      specialization_bonus + complexity_bonus + quality_bonus + bert_bonus)
    ...
    def predict_winners(self, matched_firms, project_details, similar_projects):
        ...
        for firm_info in matched_firms:
            ...
            firm_scores.append({
                ...
                'bert_similarity': np.mean([sp['bert_similarity'] for sp in similar_projects]) if similar_projects else 0.0
            })
    ...
    def run_test(self, start_bulletin, end_bulletin, test_name):
        ...
        print(f"=== PHASE 2.2a BERT SYSTEM - {test_name} ===")
        ...
        print(f"\n{'='*60}")
        print(f"PHASE 2.2a BERT RESULTS SUMMARY - {test_name}")
        print(f"{'='*60}")
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            summary_data = {
                ...
                'Value': [
                    ...
                    "Phase 2.2a BERT",
                    "TESTING",
                    "BERT/Sentence Transformers, Advanced Embeddings, Enhanced Similarity"
                ]
            }
            ...
            for result in results['results']:
                ...
                for i, pred in enumerate(predictions, 1):
                    detailed_data.append({
                        ...
                        'BERT Similarity': pred.get('bert_similarity', 0.0),
...
def main():
    """Main function to run the Phase 2.2a BERT system"""
    system = Phase22aBERTSystem()
    
    # Test Phase 2.2a on both ranges
    print("Running Phase 2.2a BERT System")
    results1 = system.run_test(180, 190, "Phase22a_BERT_PTB180_190")
    results2 = system.run_test(190, 200, "Phase22a_BERT_PTB190_200")
    
    # Print Phase 2.2a summary
    print(f"\n{'='*80}")
    print(f"PHASE 2.2a BERT SYSTEM - TESTING SUMMARY")
    print(f"{'='*80}")
    ...
    print(f"System: Phase 2.2a (BERT/Sentence Transformers)")
