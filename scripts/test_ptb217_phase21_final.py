#!/usr/bin/env python3
"""
Phase 2.1 System - PTB217 Test (FINAL CORRECTED VERSION)
Testing the baseline Phase 2.1 system on new bulletin PTB217 with correct project count
"""
...
class Phase21PTB217TestFinal:
    ...
    def extract_all_projects_from_bulletin_final(self, bulletin_text):
        """FINAL: Extract all projects from PTB217 with deduplication"""
        print("🔄 Extracting projects from PTB217 (FINAL VERSION)...")
        
        # Find all job numbers first
        job_matches = re.findall(r'Job No\.\s*([A-Z]-\d+-\d+-\d+)', bulletin_text)
        unique_jobs = list(dict.fromkeys(job_matches))  # Remove duplicates while preserving order
        
        print(f"🔍 Found {len(job_matches)} job references, {len(unique_jobs)} unique jobs")
        
        projects = []
        for i, job_number in enumerate(unique_jobs):
            # Find the full text for this job number
            pattern = rf'Job No\.\s*{re.escape(job_number)},([^J]*(?:Job No\.|$))'
            match = re.search(pattern, bulletin_text, re.DOTALL)
            
            if match:
                full_text = f"Job No. {job_number}," + match.group(1)
                ...
                desc_patterns = [
                    rf'Job No\.\s*{re.escape(job_number)},\s*([^,]+)',
                    rf'Job\s*#\s*{re.escape(job_number)}[^,]*,\s*([^,]+)',
                    rf'{re.escape(job_number)}[^,]*,\s*([^,]+)'
                ]
                ...
                print(f"  📋 Project {i+1}: {normalized_job_number} - {description[:50]}... (Quality: {quality_score:.1f})")
    ...
    def process_ptb217_final(self):
        """Process PTB217 bulletin with final corrected extraction"""
        print(f"\n{'='*80}")
        print(f"🎯 PROCESSING PTB217 - PHASE 2.1 SYSTEM (FINAL VERSION)")
        print(f"{'='*80}")
        ...
        # Use the FINAL extraction method
        projects = self.extract_all_projects_from_bulletin_final(bulletin_text)
        
        if not projects:
            print("❌ No projects found in PTB217, stopping...")
            return None
        
        print(f"\n🎯 Found {len(projects)} unique projects in PTB217 (FINAL)")
        
        # Process projects 1-50 as requested
        projects_to_process = projects[:50]
        print(f"📋 Processing projects 1-50 as requested...")
        ...
        return project_results, len(projects)  # Return both results and total count
    
    def export_ptb217_results_final(self, results, total_projects):
        """Export PTB217 results to Excel (FINAL VERSION)"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"../results/PTB217_Phase21_Predictions_FINAL_{timestamp}.xlsx"
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            summary_data = {
                'Metric': [
                    'Bulletin',
                    'Total Projects in File',
                    'Projects Successfully Extracted',
                    'Projects Processed',
                ...
                'Value': [
                    ...
                    len(results),
                    len(results),
                    f"{(len(results)/total_projects)*100:.1f}%" if total_projects > 0 else "0%",
                    ...
                    "Top 3",
                    "Phase 2.1 Baseline (Final)",
                    "Future Bulletin Test (Final)",
                    "Corrected extraction with deduplication - Projects 1-50"
                ]
            }
...
def main():
    """Main function to test PTB217 with Phase 2.1 system (FINAL)"""
    print("🚀 STARTING PTB217 TEST WITH PHASE 2.1 SYSTEM (FINAL VERSION)")
    print("="*80)
    
    # Initialize system
    system = Phase21PTB217TestFinal()
    ...
    # Process PTB217 with final corrected extraction
    results, total_projects = system.process_ptb217_final()
    
    if results:
        # Export results
        filename = system.export_ptb217_results_final(results, total_projects)
        
        # Print summary
        print(f"\n{'='*80}")
        print(f"🎯 PTB217 TEST COMPLETE - PHASE 2.1 SYSTEM (FINAL)")
        print(f"{'='*80}")
        print(f"📊 Total Projects in File: {total_projects}")
        print(f"📋 Projects Successfully Extracted: {len(results)}")
        print(f"📈 Extraction Success Rate: {(len(results)/total_projects)*100:.1f}%")
        print(f"📄 Results File: {filename}")
        print(f"✅ System: Phase 2.1 Baseline (Final)")
        print(f"🎯 Purpose: Future Bulletin Prediction Test (Final)")
        print(f"📝 Note: Corrected extraction with deduplication - Projects 1-50")
