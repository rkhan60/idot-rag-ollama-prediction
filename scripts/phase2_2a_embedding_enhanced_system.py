#!/usr/bin/env python3
"""
Phase 2.2a: Embedding-Enhanced RAG System
Implements hybrid TF-IDF + Sentence-BERT similarity for enhanced project matching
"""
...
# Try to import sentence-transformers, fallback to TF-IDF only if not available
try:
    from sentence_transformers import SentenceTransformer
    EMBEDDINGS_AVAILABLE = True
    print("✅ Sentence-BERT embeddings available")
except ImportError:
    EMBEDDINGS_AVAILABLE = False
    print("⚠️  Sentence-BERT not available, using TF-IDF only")

class Phase22aEmbeddingEnhancedSystem:
    def __init__(self):
        self.firms_data = {}
        self.prequal_lookup = {}
        ...
        # Phase 2.1: Surgical enhancement tracking
        ...
        # NEW: Phase 2.2a embedding components
        self.embedding_model = None
        self.project_embeddings = None
        self.hybrid_weights = {'tfidf': 0.6, 'embedding': 0.4}
    ...
    def load_data(self):
        ...
        # Phase 2.1: Build surgical enhancement data
    ...
    def get_eligible_firms_by_prequalification_enhanced(self, required_prequals):
        ...
        for prequal in required_prequals:
            if prequal in self.prequal_lookup:
                ...
                # ENHANCED: Improved fuzzy matching with higher threshold
    ...
    def build_rag_knowledge_base(self):
        """Build RAG knowledge base with hybrid TF-IDF + embeddings"""
        print("🔄 Building hybrid RAG knowledge base...")
        ...
        # Build TF-IDF knowledge base
        self.vectorizer = TfidfVectorizer(max_features=1000, stop_words='english')
        self.rag_knowledge_base = self.vectorizer.fit_transform(project_descriptions)
        self.project_metadata = project_metadata
        
        # NEW: Build embedding knowledge base if available
        if EMBEDDINGS_AVAILABLE:
            print("🔄 Loading Sentence-BERT model...")
            try:
                self.embedding_model = SentenceTransformer('all-MiniLM-L6-v2')
                print("🔄 Computing project embeddings...")
                self.project_embeddings = self.embedding_model.encode(project_descriptions, convert_to_tensor=True)
                print(f"✅ Built hybrid RAG knowledge base for {len(project_descriptions)} projects")
                print(f"  TF-IDF: Available")
                print(f"  Sentence-BERT: Available")
            except Exception as e:
                print(f"⚠️  Embedding model failed to load: {e}")
                print(f"  Falling back to TF-IDF only")
                self.embedding_model = None
                self.project_embeddings = None
        else:
            print(f"✅ Built TF-IDF RAG knowledge base for {len(project_descriptions)} projects")
            print(f"  TF-IDF: Available")
            print(f"  Sentence-BERT: Not available")
    
    def retrieve_similar_projects_enhanced(self, project_description, project_context, top_k=5):
        """Enhanced RAG with hybrid TF-IDF + embedding similarity"""
        ...
        tfidf_similarities = cosine_similarity(query_vector, self.rag_knowledge_base).flatten()
        
        # Calculate embedding similarity if available
        embedding_similarities = None
        if self.embedding_model is not None and self.project_embeddings is not None:
            try:
                query_embedding = self.embedding_model.encode([project_description], convert_to_tensor=True)
                embedding_similarities = cosine_similarity(query_embedding, self.project_embeddings).flatten()
            except Exception as e:
        ...
        if embedding_similarities is not None:
            # Hybrid similarity: 0.6 * TF-IDF + 0.4 * embedding
            combined_similarities = (
                self.hybrid_weights['tfidf'] * tfidf_similarities +
                self.hybrid_weights['embedding'] * embedding_similarities
            )
            similarity_type = "Hybrid (TF-IDF + Sentence-BERT)"
        else:
            # Fallback to TF-IDF only
            combined_similarities = tfidf_similarities
            similarity_type = "TF-IDF only"
        
        top_indices = combined_similarities.argsort()[-top_k*3:][::-1]
        
        similar_projects = []
        for idx in top_indices:
            if combined_similarities[idx] > 0.1:
                metadata = self.project_metadata[idx]
                
                base_similarity = combined_similarities[idx]
                ...
                similar_projects.append({
                    ...
                    'tfidf_similarity': tfidf_similarities[idx] if embedding_similarities is not None else base_similarity,
                    'embedding_similarity': embedding_similarities[idx] if embedding_similarities is not None else None,
    ...
    def calculate_surgical_score(self, firm_info, project_details, similar_projects):
        """Calculate score using surgical enhancements to Phase 2.1"""
    ...
    def run_test(self, start_bulletin, end_bulletin, test_name):
        ...
        print(f"\n{'='*80}")
        print(f"🎯 RUNNING {test_name} WITH PHASE 2.2A EMBEDDING ENHANCEMENTS")
        print(f"{'='*80}")
        ...
        filename = f"../results/{test_name}_Phase22a_{timestamp}.xlsx"
        
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            # Summary sheet
            summary_data = {
                'Metric': ['Test Name', 'Total Projects', 'Successful Predictions', 'Accuracy', 'System Version', 'Enhancements Applied'],
                'Value': [test_name, total_projects, successful_predictions, f"{accuracy:.1f}%", "Phase 2.2a Embedding Enhanced", "Hybrid TF-IDF + Sentence-BERT, Surgical Enhancements"]
            }
...
def main():
    """Main function to test Phase 2.2a embedding enhanced system"""
    print("🚀 TESTING PHASE 2.2A: EMBEDDING-ENHANCED RAG SYSTEM")
    print("="*80)
    
    tester = Phase22aEmbeddingEnhancedSystem()
    
    # Test 1: PTB180-190
    print("\n🎯 TEST 1: PTB180-190")
    results1 = tester.run_test(180, 190, "PTB180_190_Phase22a")
    
    # Test 2: PTB190-200
    print("\n🎯 TEST 2: PTB190-200")
    results2 = tester.run_test(190, 200, "PTB190_200_Phase22a")
    ...
    if results1 and results2:
        ...
        print(f"\n📊 PHASE 2.2A EMBEDDING ENHANCED RESULTS:")
        ...
        if ptb180_190_improvement > 0 and ptb190_200_improvement > 0:
            ...
            print(f"   Both ranges show improvement - Phase 2.2a is successful")
        elif average_improvement > 0:
            print(f"\n⚠️  PARTIAL IMPROVEMENT:")
            print(f"   Average improved but not both ranges - Needs analysis")
        else:
            print(f"\n❌ NO IMPROVEMENT:")
            print(f"   Rolling back to Surgical Enhancements baseline")
