...
class HybridPhase2Phase3System:
    def __init__(self):
        self.firms_data = {}
        self.prequal_lookup = {}
        ...
        # Phase 3: Selective enhancements (excluding partnership patterns)
        self.firm_specializations = defaultdict(Counter)
        self.firm_success_rates = {}
        self.project_complexity_indicators = {
            'urgency': ['urgent', 'emergency', 'immediate', 'critical', 'rush'],
            'complexity': ['complex', 'advanced', 'sophisticated', 'multi-phase', 'major'],
            'scale': ['large', 'extensive', 'comprehensive', 'statewide', 'major'],
            'political': ['sensitive', 'controversial', 'public', 'community', 'stakeholder']
        }
    ...
    def load_data(self):
        ...
        # Phase 3: Build firm capability profiles (selective)
    ...
    def build_firm_capability_profiles(self):
        """Phase 3: Build firm capability profiles (selective - no partnerships)"""
        print("Building firm capability profiles...")
        
        firm_wins = Counter()
        firm_category_wins = defaultdict(Counter)
        ...
        for firm_name, wins in firm_wins.items():
            self.firm_success_rates[firm_name] = wins / total_awards
            self.firm_specializations[firm_name] = firm_category_wins[firm_name]
        
        print(f"Built capability profiles for {len(self.firm_success_rates)} firms")
    ...
    def extract_contextual_features(self, project_description):
        """Phase 3: Extract contextual features from project description"""
        description_lower = project_description.lower()
        
        contextual_features = {
            'urgency': sum(1 for word in self.project_complexity_indicators['urgency'] if word in description_lower),
            'complexity': sum(1 for word in self.project_complexity_indicators['complexity'] if word in description_lower),
            'scale': sum(1 for word in self.project_complexity_indicators['scale'] if word in description_lower),
            'political': sum(1 for word in self.project_complexity_indicators['political'] if word in description_lower)
        }
    ...
    def extract_all_projects_from_bulletin(self, bulletin_text):
        ...
        for i, match in enumerate(matches):
            ...
            contextual_features = self.extract_contextual_features(description)
    ...
    def retrieve_similar_projects_enhanced(self, project_description, project_context, top_k=5):
        ...
        for idx in top_indices:
            if similarities[idx] > 0.1:
                ...
                # Phase 3: Add contextual similarity
                contextual_similarity = self.calculate_contextual_similarity(project_context, metadata)
                
                combined_similarity = (base_similarity * temporal_weight * 
                                     complexity_score * (1 + type_similarity * 0.4) *
                                     (1 + contextual_similarity * 0.2))
    ...
    def calculate_contextual_similarity(self, project_context, historical_metadata):
        """Phase 3: Calculate contextual similarity"""
        current_features = project_context.get('contextual_features', {})
        
        # Simplified contextual similarity based on complexity indicators
        current_complexity = current_features.get('complexity', 0)
        current_urgency = current_features.get('urgency', 0)
        
        # Historical project complexity (simplified)
        historical_desc = historical_metadata['description'].lower()
        historical_complexity = sum(1 for word in self.project_complexity_indicators['complexity'] if word in historical_desc)
        historical_urgency = sum(1 for word in self.project_complexity_indicators['urgency'] if word in historical_desc)
        
        # Calculate similarity
        complexity_sim = 1.0 - abs(current_complexity - historical_complexity) / max(1, max(current_complexity, historical_complexity))
        urgency_sim = 1.0 - abs(current_urgency - historical_urgency) / max(1, max(current_urgency, historical_urgency))
        
        return (complexity_sim + urgency_sim) / 2
    
    def calculate_hybrid_score(self, firm_info, project_details, similar_projects):
        """Hybrid scoring: Phase 2 + selective Phase 3 components"""
        ...
        # Phase 2: Temporal-weighted similar project experience
        ...
        if similar_projects:
            ...
            if avg_complexity > 1.2:
                complexity_bonus = 5 + np.random.randint(0, 5)
        
        # Phase 3: Selective enhancements (no partnerships)
        capability_bonus = 0
        if firm_name in self.firm_success_rates:
            success_rate = self.firm_success_rates[firm_name]
            capability_bonus = success_rate * 20  # Scale success rate to bonus
        
        specialization_bonus_2 = 0
        if firm_name in self.firm_specializations:
            project_type = self.determine_project_type(project_details['description'])
            if project_type in self.firm_specializations[firm_name]:
                specialization_bonus_2 = self.firm_specializations[firm_name][project_type] * 2
        
        contextual_bonus = 0
        contextual_features = project_details.get('contextual_features', {})
        if contextual_features.get('complexity', 0) > 0 or contextual_features.get('urgency', 0) > 0:
            contextual_bonus = 3 + np.random.randint(0, 4)
        
        # Calculate final hybrid score
        final_score = (base_score - distance_penalty + capacity_bonus + 
                      historical_bonus + recent_bonus + similar_project_bonus + 
                      specialization_bonus + complexity_bonus + capability_bonus + 
                      specialization_bonus_2 + contextual_bonus)
    ...
    def predict_winners(self, matched_firms, project_details, similar_projects):
        """Predict top 3 winning firms with hybrid enhancements"""
        ...
        for firm_info in matched_firms:
            score = self.calculate_hybrid_score(firm_info, project_details, similar_projects)
            ...
            firm_scores.append({
                ...
                'success_rate': self.firm_success_rates.get(firm_name, 0),
                'specialization_count': sum(self.firm_specializations.get(firm_name, {}).values())
            })
    ...
    def run_test(self, start_bulletin, end_bulletin, test_name):
        ...
        print(f"=== HYBRID PHASE 2 + PHASE 3 SYSTEM - {test_name} ===")
        ...
        print(f"\n{'='*60}")
        print(f"HYBRID RESULTS SUMMARY - {test_name}")
        print(f"{'='*60}")
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            summary_data = {
                ...
                'Value': [
                    ...
                    "Top 3",
                    "33.3% (1/3)",
                    "Hybrid Phase 2 + Phase 3",
                    "TESTING",
                    "Phase 2 Core + Selective Phase 3 (Firm Profiling, Contextual Features)"
                ]
            }
            ...
            for result in results['results']:
                ...
                for i, pred in enumerate(predictions, 1):
                    detailed_data.append({
                        ...
                        'Specialization Count': pred.get('specialization_count', 0),
...
def main():
    ...
    system = HybridPhase2Phase3System()
    
    # Test hybrid system on both ranges
    print("Running Hybrid Phase 2 + Phase 3 System")
    results1 = system.run_test(180, 190, "Hybrid_PTB180_190")
    results2 = system.run_test(190, 200, "Hybrid_PTB190_200")
    
    # Print hybrid summary
    print(f"\n{'='*80}")
    print(f"HYBRID PHASE 2 + PHASE 3 SYSTEM - TESTING SUMMARY")
    print(f"{'='*80}")
    ...
    print(f"System: Hybrid (Phase 2 Core + Selective Phase 3)")
    print(f"Next Step: Compare with Production Phase 2")
