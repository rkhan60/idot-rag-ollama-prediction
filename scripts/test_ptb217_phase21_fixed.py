#!/usr/bin/env python3
"""
Phase 2.1 System - PTB217 Test (FIXED VERSION)
Testing the baseline Phase 2.1 system on new bulletin PTB217 with fixed extraction
"""
...
class Phase21PTB217TestFixed:
    ...
    def extract_all_projects_from_bulletin_fixed(self, bulletin_text):
        """FIXED: Extract all projects from PTB217 with improved patterns"""
        print("🔄 Extracting projects from PTB217 (FIXED VERSION)...")
        projects = []
        
        # FIXED: Enhanced job pattern matching for PTB217 format
        # PTB217 format: "Job No. V-91-010-25, Description..."
        job_patterns = [
            # Pattern 1: Job No. A-XX-XXX-XX, Description
            r'(Job No\.\s*([A-Z]-\d+-\d+-\d+),\s*([^,]+?)(?:,|\.).*?)(?=Job No\.|$)',
            # Pattern 2: Job No. A-XX-XXX-XX, Description (alternative)
            r'(Job No\.\s*([A-Z]-\d+-\d+-\d+),\s*([^.]*?)(?:\.|$).*?)(?=Job No\.|$)',
        ...
        print(f"🔍 Found {len(all_matches)} job matches in PTB217 (FIXED)")
        ...
        for i, match in enumerate(all_matches):
            ...
            # Enhanced description extraction (if not already extracted)
    ...
    def process_ptb217_fixed(self):
        """Process PTB217 bulletin with fixed extraction"""
        print(f"\n{'='*80}")
        print(f"🎯 PROCESSING PTB217 - PHASE 2.1 SYSTEM (FIXED VERSION)")
        print(f"{'='*80}")
        ...
        projects = self.extract_all_projects_from_bulletin_fixed(bulletin_text)
        
        if not projects:
            print("❌ No projects found in PTB217, stopping...")
            return None
        
        print(f"\n🎯 Found {len(projects)} projects in PTB217 (FIXED)")
        
        # Process only first 10 projects for demonstration (to avoid overwhelming output)
        projects_to_process = projects[:10]
        print(f"📋 Processing first {len(projects_to_process)} projects for demonstration...")
    ...
    def export_ptb217_results_fixed(self, results, total_projects):
        """Export PTB217 results to Excel (FIXED VERSION)"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"../results/PTB217_Phase21_Predictions_FIXED_{timestamp}.xlsx"
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            summary_data = {
                ...
                'Value': [
                    ...
                    "Top 3",
                    "Phase 2.1 Baseline (Fixed)",
                    "Future Bulletin Test (Fixed Extraction)",
                    "Fixed extraction pattern for PTB217 format"
                ]
            }
...
def main():
    """Main function to test PTB217 with Phase 2.1 system (FIXED)"""
    print("🚀 STARTING PTB217 TEST WITH PHASE 2.1 SYSTEM (FIXED VERSION)")
    print("="*80)
    
    # Initialize system
    system = Phase21PTB217TestFixed()
    ...
    # Process PTB217 with fixed extraction
    results, total_projects = system.process_ptb217_fixed()
    
    if results:
        # Export results
        filename = system.export_ptb217_results_fixed(results, total_projects)
        
        # Print summary
        print(f"\n{'='*80}")
        print(f"🎯 PTB217 TEST COMPLETE - PHASE 2.1 SYSTEM (FIXED)")
        print(f"{'='*80}")
        ...
        print(f"✅ System: Phase 2.1 Baseline (Fixed)")
        print(f"🎯 Purpose: Future Bulletin Prediction Test (Fixed)")
        print(f"📝 Note: Fixed extraction pattern for PTB217 format")
