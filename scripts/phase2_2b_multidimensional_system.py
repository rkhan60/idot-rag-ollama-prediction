...
class Phase22bMultidimensionalSystem:
    def __init__(self):
        self.firms_data = {}
        self.prequal_lookup = {}
        ...
        # Phase 2.2b: Multi-dimensional similarity features
        self.project_size_scores = {}
        self.project_urgency_scores = {}
        self.regional_similarity_weights = {}
    ...
    def load_data(self):
        ...
        # Phase 2.2b: Build multi-dimensional features
        self.build_project_size_scores()
        self.build_project_urgency_scores()
        self.build_regional_similarity_weights()
    ...
    def build_project_size_scores(self):
        """Phase 2.2b: Build project size scoring"""
        print("Building project size scores...")
        
        size_indicators = {
            'large': ['major', 'extensive', 'comprehensive', 'large-scale', 'multi-million', 'significant'],
            'medium': ['typical', 'standard', 'moderate', 'medium-scale'],
            'small': ['minor', 'small', 'limited', 'basic', 'simple']
        }
        ...
        for i, award in enumerate(self.award_structure):
            ...
            if description:
                ...
                large_count = sum(1 for word in size_indicators['large'] if word in description_lower)
                size_score += large_count * 0.4
                
                medium_count = sum(1 for word in size_indicators['medium'] if word in description_lower)
                size_score += medium_count * 0.1
                
                small_count = sum(1 for word in size_indicators['small'] if word in description_lower)
                size_score -= small_count * 0.3
                
                self.project_size_scores[i] = max(0.5, size_score)
            else:
                self.project_size_scores[i] = 1.0
        
        print(f"Built size scores for {len(self.project_size_scores)} projects")
    
    def build_project_urgency_scores(self):
        """Phase 2.2b: Build project urgency scoring"""
        print("Building project urgency scores...")
        
        urgency_indicators = {
            'high_urgency': ['urgent', 'emergency', 'critical', 'immediate', 'expedited', 'fast-track'],
            'medium_urgency': ['timely', 'scheduled', 'planned', 'routine'],
            'low_urgency': ['long-term', 'future', 'planning', 'study']
        }
        ...
        for i, award in enumerate(self.award_structure):
            ...
            if description:
                ...
                high_count = sum(1 for word in urgency_indicators['high_urgency'] if word in description_lower)
                urgency_score += high_count * 0.3
                
                medium_count = sum(1 for word in urgency_indicators['medium_urgency'] if word in description_lower)
                urgency_score += medium_count * 0.1
                
                low_count = sum(1 for word in urgency_indicators['low_urgency'] if word in description_lower)
                urgency_score -= low_count * 0.2
                
                self.project_urgency_scores[i] = max(0.5, urgency_score)
            else:
                self.project_urgency_scores[i] = 1.0
        
        print(f"Built urgency scores for {len(self.project_urgency_scores)} projects")
    
    def build_regional_similarity_weights(self):
        """Phase 2.2b: Build regional similarity weights"""
        print("Building regional similarity weights...")
        
        # Define regional clusters
        regional_clusters = {
            'northern': ['Region One', 'Region Two'],
            'central': ['Region Three', 'Region Four'],
            'southern': ['Region Five']
        }
        ...
        for i, award in enumerate(self.award_structure):
            ...
            if description:
                ...
                region_match = re.search(r'Region\s+(\w+)', description, re.IGNORECASE)
                if region_match:
                    region = f"Region {region_match.group(1)}"
                    # Assign regional weight based on cluster
                    for cluster, regions in regional_clusters.items():
                        if region in regions:
                            self.regional_similarity_weights[i] = cluster
                            break
                    else:
                        self.regional_similarity_weights[i] = 'general'
                else:
                    self.regional_similarity_weights[i] = 'general'
            else:
                self.regional_similarity_weights[i] = 'general'
        
        print(f"Built regional weights for {len(self.regional_similarity_weights)} projects") 
    ...
    def build_rag_knowledge_base(self):
        """Phase 2.2b: Build advanced RAG knowledge base with multi-dimensional features"""
        print("Building advanced RAG knowledge base with multi-dimensional features...")
        ...
        for i, award in enumerate(self.award_structure):
            ...
            if description and job_number:
                ...
                project_metadata.append({
                    ...
                    'size_score': self.project_size_scores.get(i, 1.0),
                    'urgency_score': self.project_urgency_scores.get(i, 1.0),
                    'regional_cluster': self.regional_similarity_weights.get(i, 'general'),
                    'award_index': i
                })
        ...
        print(f"Built advanced RAG knowledge base with multi-dimensional features for {len(project_descriptions)} projects")
    
    def retrieve_similar_projects_enhanced(self, project_description, project_context, top_k=5):
        """Phase 2.2b: Advanced RAG with multi-dimensional similarity"""
        ...
        for idx in top_indices:
            if similarities[idx] > 0.1:
                ...
                size_score = metadata.get('size_score', 1.0)
                urgency_score = metadata.get('urgency_score', 1.0)
                regional_cluster = metadata.get('regional_cluster', 'general')
                
                # Phase 2.2b: Multi-dimensional similarity calculation
                type_similarity = self.calculate_project_type_similarity(project_context, metadata)
                size_similarity = self.calculate_size_similarity(project_context, metadata)
                urgency_similarity = self.calculate_urgency_similarity(project_context, metadata)
                regional_similarity = self.calculate_regional_similarity(project_context, metadata)
                
                # Enhanced combined similarity with multi-dimensional weights
                combined_similarity = (base_similarity * temporal_weight * 
                                     complexity_score * (1 + type_similarity * 0.3) *
                                     (1 + size_similarity * 0.2) * (1 + urgency_similarity * 0.2) *
                                     (1 + regional_similarity * 0.3))
    ...
    def calculate_project_type_similarity(self, project_context, historical_metadata):
        ...
        current_desc = project_context.get('description', '')
        historical_desc = historical_metadata.get('description', '')
        
        current_type = self.categorize_project_type(current_desc.lower() if current_desc else '')
        historical_type = self.categorize_project_type(historical_desc.lower() if historical_desc else '')
    ...
    def calculate_size_similarity(self, project_context, historical_metadata):
        """Phase 2.2b: Calculate project size similarity"""
        current_desc = project_context.get('description', '').lower()
        historical_desc = historical_metadata.get('description', '').lower()
        ...
        current_size = 'medium'
        historical_size = 'medium'
        
        for size, indicators in size_indicators.items():
            if any(word in current_desc for word in indicators):
                current_size = size
            if any(word in historical_desc for word in indicators):
                historical_size = size
        
        if current_size == historical_size:
            return 0.5
        elif (current_size == 'large' and historical_size == 'medium') or (current_size == 'medium' and historical_size == 'large'):
            return 0.3
        else:
            return 0.0
    
    def calculate_urgency_similarity(self, project_context, historical_metadata):
        """Phase 2.2b: Calculate project urgency similarity"""
        current_desc = project_context.get('description', '').lower()
        historical_desc = historical_metadata.get('description', '').lower()
        
        urgency_indicators = {
            'high': ['urgent', 'emergency', 'critical', 'immediate', 'expedited', 'fast-track'],
            'medium': ['timely', 'scheduled', 'planned', 'routine'],
            'low': ['long-term', 'future', 'planning', 'study']
        }
        
        current_urgency = 'medium'
        historical_urgency = 'medium'
        
        for urgency, indicators in urgency_indicators.items():
            if any(word in current_desc for word in indicators):
                current_urgency = urgency
            if any(word in historical_desc for word in indicators):
                historical_urgency = urgency
        
        if current_urgency == historical_urgency:
            return 0.4
        elif (current_urgency == 'high' and historical_urgency == 'medium') or (current_urgency == 'medium' and historical_urgency == 'high'):
            return 0.2
        else:
            return 0.0
    
    def calculate_regional_similarity(self, project_context, historical_metadata):
        """Phase 2.2b: Calculate regional similarity"""
        current_region = project_context.get('region_district', '')
        historical_cluster = historical_metadata.get('regional_cluster', 'general')
        
        if not current_region or historical_cluster == 'general':
            return 0.0
        
        # Extract region from current project
        region_match = re.search(r'Region\s+(\w+)', current_region, re.IGNORECASE)
        if not region_match:
            return 0.0
        
        current_region_name = f"Region {region_match.group(1)}"
        ...
        # Find current project's cluster
        ...
        if current_cluster == historical_cluster:
            return 0.6
        elif current_cluster != 'general' and historical_cluster != 'general':
            return 0.2
        else:
            return 0.0
    ...
    def calculate_phase2_score(self, firm_info, project_details, similar_projects):
        """Phase 2.2b: Advanced scoring with multi-dimensional enhancements"""
        ...
        # Phase 2.2b: Enhanced similar project experience bonus with multi-dimensional features
        ...
        for similar_project in similar_projects:
            if similar_project['metadata']['selected_firm'] == firm_name:
                ...
                size_score = similar_project['size_score']
                urgency_score = similar_project['urgency_score']
                regional_cluster = similar_project['regional_cluster']
                
                type_similarity = similar_project['type_similarity']
                size_similarity = similar_project['size_similarity']
                urgency_similarity = similar_project['urgency_similarity']
                regional_similarity = similar_project['regional_similarity']
                
                base_bonus = 12 + np.random.randint(0, 8)
                weighted_bonus = (base_bonus * temporal_weight * complexity_score * 
                                size_score * urgency_score * (1 + type_similarity * 0.4) *
                                (1 + size_similarity * 0.3) * (1 + urgency_similarity * 0.3) *
                                (1 + regional_similarity * 0.4))
                similar_project_bonus += weighted_bonus
                break
        
        # Project type specialization bonus (enhanced with Phase 2.2b)
        ...
        # Phase 2.2b: Multi-dimensional complexity bonus
        complexity_bonus = 0
        if similar_projects:
            avg_complexity = np.mean([sp['complexity_score'] for sp in similar_projects])
            avg_size = np.mean([sp['size_score'] for sp in similar_projects])
            avg_urgency = np.mean([sp['urgency_score'] for sp in similar_projects])
        ...
        # Phase 2.2b: Regional expertise bonus
        regional_bonus = 0
        if similar_projects:
            avg_regional_similarity = np.mean([sp['regional_similarity'] for sp in similar_projects])
            if avg_regional_similarity > 0.3:
                regional_bonus = 4 + np.random.randint(0, 4)
        
        # Calculate final score
        final_score = (base_score - distance_penalty + capacity_bonus + 
                      historical_bonus + recent_bonus + similar_project_bonus + 
                      specialization_bonus + complexity_bonus + quality_bonus + regional_bonus)
    ...
    def predict_winners(self, matched_firms, project_details, similar_projects):
        """Predict top 3 winning firms with Phase 2.2b enhancements"""
        ...
        for firm_info in matched_firms:
            ...
            firm_scores.append({
                ...
                'size_score': np.mean([sp['size_score'] for sp in similar_projects]) if similar_projects else 1.0,
                'urgency_score': np.mean([sp['urgency_score'] for sp in similar_projects]) if similar_projects else 1.0,
                'regional_similarity': np.mean([sp['regional_similarity'] for sp in similar_projects]) if similar_projects else 0.0,
    ...
    def run_test(self, start_bulletin, end_bulletin, test_name):
        ...
        print(f"=== PHASE 2.2b MULTIDIMENSIONAL SYSTEM - {test_name} ===")
        ...
        print(f"\n{'='*60}")
        print(f"PHASE 2.2b MULTIDIMENSIONAL RESULTS SUMMARY - {test_name}")
        print(f"{'='*60}")
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            summary_data = {
                ...
                'Value': [
                    ...
                    "Top 3",
                    "33.3% (1/3)",
                    "Phase 2.2b Multidimensional",
                    "TESTING",
                    "Multi-dimensional Similarity, Enhanced RAG, Regional Clustering, Size/Urgency Analysis"
                ]
            }
            ...
            for result in results['results']:
                ...
                for i, pred in enumerate(predictions, 1):
                    detailed_data.append({
                        ...
                        'Size Score': pred.get('size_score', 1.0),
                        'Urgency Score': pred.get('urgency_score', 1.0),
                        'Regional Similarity': pred.get('regional_similarity', 0.0),
...
def main():
    """Main function to run the Phase 2.2b Multidimensional system"""
    system = Phase22bMultidimensionalSystem()
    
    # Test Phase 2.2b on both ranges
    print("Running Phase 2.2b Multidimensional System")
    results1 = system.run_test(180, 190, "Phase22b_Multidimensional_PTB180_190")
    results2 = system.run_test(190, 200, "Phase22b_Multidimensional_PTB190_200")
    
    # Print Phase 2.2b summary
    print(f"\n{'='*80}")
    print(f"PHASE 2.2b MULTIDIMENSIONAL SYSTEM - TESTING SUMMARY")
    print(f"{'='*80}")
    ...
    print(f"System: Phase 2.2b (Multi-dimensional Similarity)")
