#!/usr/bin/env python3
"""
PTB Enrichment Processor
Extract prequalifications from PTB files and enrich award.xlsx Prequals column
"""
...
class PTBEnrichmentProcessor:
    def __init__(self):
        self.award_file = '../data/award.xlsx'
        self.ptb_directory = '../data/'
        self.prequal_lookup_file = '../data/prequal_lookup.json'
        self.output_file = '../data/award_enriched.xlsx'
        self.log_file = f"enrichment_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    def extract_job_numbers_from_ptb(self, ptb_text):
        """Extract job numbers from PTB text"""
        job_numbers = []
        
        # Common job number patterns
        patterns = [
            r'Job\s*#?\s*([A-Z]-\d{2}-\d{3}-\d{2})',  # D-91-516-11
            r'([A-Z]-\d{2}-\d{3}-\d{2})',  # Standalone job numbers
            r'Project\s*#?\s*([A-Z]-\d{2}-\d{3}-\d{2})',  # Project numbers
        ]
        ...
        return list(set(job_numbers))
    
    def extract_prequalifications_from_ptb(self, ptb_text):
        """Extract prequalification requirements from PTB text"""
        prequalifications = []
        
        # Look for Exhibit A section
        exhibit_patterns = [
        ...
        if not exhibit_text:
            # If no Exhibit A found, look for prequalification keywords in entire text
            prequal_keywords = ['prequalification', 'qualification', 'required', 'must have']
            if any(keyword in ptb_text.lower() for keyword in prequal_keywords):
                exhibit_text = ptb_text
    ...
    def process_single_ptb(self, ptb_number):
        ...
        # Extract job numbers
        job_numbers = self.extract_job_numbers_from_ptb(ptb_text)
        self.log_message(f"📋 PTB{ptb_number}: Found {len(job_numbers)} job numbers: {job_numbers}")
        
        # Extract prequalifications
        prequalifications = self.extract_prequalifications_from_ptb(ptb_text)
        self.log_message(f"📝 PTB{ptb_number}: Found {len(prequalifications)} prequalifications: {prequalifications}")
        
        # Validate prequalifications
        validated_prequals, errors = self.validate_prequalifications(prequalifications)
        if errors:
            self.log_message(f"⚠️ PTB{ptb_number}: Validation errors: {errors}")
            self.enrichment_stats['validation_errors'] += len(errors)
        
        # Update statistics
        self.enrichment_stats['processed_ptb_files'] += 1
        if validated_prequals:
            self.enrichment_stats['successful_extractions'] += 1
            self.enrichment_stats['prequalifications_extracted'] += len(validated_prequals)
        ...
        return {
            ...
            'ptb_text_sample': ptb_text[:500] + "..." if len(ptb_text) > 500 else ptb_text
        }
    
    def match_job_numbers_to_award(self, ptb_results):
        """Match PTB job numbers to award.xlsx job numbers"""
        self.log_message(f"🔗 Matching job numbers to award data...")
        
        matches = []
        award_job_numbers = set(self.award_df['Job #'].dropna().astype(str))
        
        for ptb_result in ptb_results:
            if ptb_result:
                ptb_number = ptb_result['ptb_number']
                ptb_job_numbers = ptb_result['job_numbers']
                prequalifications = ptb_result['prequalifications']
                
                for job_number in ptb_job_numbers:
    ...
    def generate_enrichment_report(self, ptb_results, matches, updated_df):
        ...
        report = {
            ...
            'enrichment_summary': {
                ...
                'successful_extractions': self.enrichment_stats['successful_extractions'],
            ...
            'ptb_results': ptb_results,
            'job_matches': matches
        }
        
        # Save report
        report_filename = f"enrichment_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        self.log_message(f"  • Successful Extractions: {self.enrichment_stats['successful_extractions']}")
    ...
    def run_complete_enrichment(self):
        """Run the complete PTB enrichment process"""
        self.log_message("🚀 STARTING PTB ENRICHMENT PROCESS")
        self.log_message("=" * 80)
        ...
        for ptb_number in ptb_numbers:
            ...
            ptb_results.append(result)
        
        # Match job numbers to award data
        matches = self.match_job_numbers_to_award(ptb_results)
        
        # Update award file
        updated_df = self.update_award_file(matches)
        
        # Generate report
        report = self.generate_enrichment_report(ptb_results, matches, updated_df)
        
        self.log_message("🎯 ENRICHMENT PROCESS COMPLETE!")
        self.log_message("=" * 80)
...
def main():
    ...
    print("🚀 PTB ENRICHMENT PROCESSOR")
    print("=" * 80)
    
    processor = PTBEnrichmentProcessor()
