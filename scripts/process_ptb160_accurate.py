#!/usr/bin/env python3
"""
Accurate PTB160 Processor - Complete Prequalification Names
==========================================================
Process PTB160 with accurate extraction of complete prequalification names.
"""
...
class AccuratePTB160Processor:
    ...
    def extract_prequalifications_accurate(self, project_text):
        """Accurate prequalification extraction that captures complete names"""
        prequalifications = []
        ...
        if categories_match:
            ...
            for line in lines:
                ...
                if line:
                    ...
                    # This regex captures the full "Category (Subcategory)" format
                    ...
                    for category, subcategory in matches:
                        ...
                        print(f"✅ Extracted complete: {full_prequal}")
        ...
        # Validate and map to lookup names
        validated_prequals = []
        for prequal in prequalifications:
            # Try to find the exact match in prequal_lookup
            matched_prequal = self.find_exact_prequal_match(prequal)
            if matched_prequal:
                validated_prequals.append(matched_prequal)
                print(f"✅ Mapped: '{prequal}' → '{matched_prequal}'")
            else:
                # Keep original if no match found
                validated_prequals.append(prequal)
                print(f"⚠️  No match found for: '{prequal}'")
        
        return list(set(validated_prequals))  # Remove duplicates
    
    def find_exact_prequal_match(self, extracted_prequal):
        """Find exact match for extracted prequalification in lookup"""
        ...
        for lookup_category in self.prequal_lookup.keys():
            # Check main category
            if self.fuzzy_match_prequal(extracted_prequal, lookup_category):
            ...
            if lookup_category in self.prequal_lookup:
                ...
                for subcat_key, subcat_data in subcategories.items():
                    ...
                    if self.fuzzy_match_prequal(extracted_prequal, full_prequal_name):
    ...
    def process_ptb160_accurate(self):
        """Process PTB160 with accurate extraction"""
        print("🔧 ACCURATE PTB160 PROCESSING")
        print("=" * 50)
        ...
        for award in ptb160_awards:
            ...
            if job_match:
                ...
                # Extract prequalifications with accurate method
                prequalifications = self.extract_prequalifications_accurate(job_text)
        ...
        # Save results
        output_filename = 'ptb_processing_reports/ptb160_accurate_enhanced_awards.json'
        ...
        report_filename = 'ptb_processing_reports/ptb160_accurate_report.json'
...
def main():
    """Main function"""
    processor = AccuratePTB160Processor()
    processor.process_ptb160_accurate()
